...
Game.prototype.draw = function (ctxTop, ctxBottom, ctxShip) {
  ...
  Object.keys(this.starSystemBottom.system[[0,0]].region[5]).map(key => {
    ...
    if (star.type === '#') {
      ...
      star.posY += (this.direction[1] / 2 * this.speed)
      star.posX += (this.direction[0] / 2 * this.speed)
  ...
  Object.keys(this.starSystemTop.system[[0,0]].region[5]).map(key => {
    ...
    if (star.type === '#') {
      ...
      star.posY += (this.direction[1] * this.speed)
      star.posX += (this.direction[0] * this.speed)
      if (star.posY + y > this.gameHeight) {
        star.posY = star.posY - this.gameHeight
      }
      if (star.posY + y < 0) {
  ...
  this.thrusters.forEach((particle, idx) =>{
    ...
    particle.h += ((particle.cH + this.direction[1])* this.speed + ((Math.random() - Math.random()) * this.direction[1]));

    particle.w += (particle.cW + this.direction[0])* this.speed;
  ...
  this.fuel -= this.speed * 50;

  let remainingFuel = this.fuel/1000000 * 100;
  ctxTop.fillStyle = 'green';
  ctxTop.fillRect(20, 90, 24, 104);
  ctxTop.clearRect(21, 91, 22, 102);
  ctxTop.fillRect(22, 192, 20, -remainingFuel);

  ctxTop.font = "30px serif";
  ctxTop.fillText("F", 20, 80);
